...
import { getAvailableBillableTimeReview, getInvoiceBasesForClient, getRecentInvoiceBases } from "@/server/invoice-bases";
...
export default async function InvoiceBasesPage({
  ...
  searchParams: Promise<{ clientId?: string | string[]; startDate?: string | string[]; endDate?: string | string[]; create?: string | string[] }>;
}) {
  ...
  const [clientOptions, result, invoiceBasesHistory] = await Promise.all([
    ...
    clientId
      ...
      : getRecentInvoiceBases(account, 20),
  ]);
                ...
                <table className="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr className="border-b border-slate-200 text-xs font-semibold uppercase tracking-wider text-slate-600">
                      ...
                      {!clientId && <th className="py-3 px-4">Client</th>}
